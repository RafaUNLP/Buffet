...
public class MenuDAOHibernateJPA extends GenericDAOHibernateJPA<Menu> implements MenuDAO{
	...
	public List<Menu> findVegetarians() {
		...
		try {
			return em.createQuery("SELECT m FROM Item WHERE m.dtype = 'vegetariano'",this.entityClass).getResultList();
		}
		finally {
			em.close();
		}
	...
	public List<Menu> findStandards() {
		EntityManager em = EMF.getEMF().createEntityManager();
		try {
			return em.createQuery("SELECT m FROM Menu WHERE m.tipo_ = estandar",this.entityClass).getResultList();
		}
		finally {
			em.close();
		}
